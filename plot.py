...
def reward_step_plot(filepaths: List[str], labels: List[str], layout_name: str):
    try:
        dataframes = {labels[i]: read_csv(filepaths[i]) for i in range(len(filepaths))}
    except :
    ...
    for label, df in dataframes.items():
        plt.plot(df["Step"],df["Average reward"], label=label)
...
def reward_capacity_plot(filepaths: List[str], capacities: List[int], layout_name: str):
    x, y = capacities, list()
    try:
        for f in filepaths:
...
def senders_predictions_to_csv(models_paths: Dict, layouts: List[int], capacities: List[int], output_filepath: str):
    ...
    for c in range(len(capacities)):
        ...
        for l in range(len(layouts)):
            ...
            goals = list(set([(i,j) for i in range(5) for j in range(5)]) - set(walls) - set([player]))
            ...
            for goal in goals:
                ...
                message = convert_message_to_decimal(agent.send_message(training=False))
...
if __name__ == '__main__':
    ...
    reward_step_plot(filepaths = [
            "./experiments/experiment_1_layout_0/experiment_1_n_1.csv",
            "./experiments/experiment_1_layout_0/experiment_1_n_2.csv"
        ],
    ...
    senders_predictions_to_csv(
        ...
        layouts=[1,3],
        capacities=[3,4,9],
        output_filepath="./experiments/results_exp_2.csv"
    )
